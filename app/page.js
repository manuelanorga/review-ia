    ...
    <div style={{ display: "grid", gridTemplateColumns: "320px 1fr", gap: 24, alignItems: "start" }} className="bgrid">
      ...
      <div style={{ background: SURF2, border: `1px solid ${BORDER}`, borderRadius: 16, overflow: "hidden" }}>
        ...
        <div style={{ padding: "16px 20px", borderBottom: `1px solid ${BORDER}` }}>
          <div style={{ fontSize: 11, color: MUTED, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 12 }}>Tono de respuesta</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {Object.entries(TONE_CONFIG).map(([key, { label, desc }]) => (
              <div key={key} onClick={() => setActiveTone(key)} style={{ padding: "12px 14px", borderRadius: 10, border: `1px solid ${activeTone === key ? Y : BORDER}`, background: activeTone === key ? "rgba(255,230,0,0.06)" : "transparent", cursor: "pointer", transition: "all 0.2s", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div>
                  <div style={{ fontSize: 13, fontWeight: activeTone === key ? 700 : 400, color: activeTone === key ? Y : LIGHT }}>{label}</div>
                  <div style={{ fontSize: 11, color: MUTED, marginTop: 2 }}>{desc}</div>
                </div>
                {activeTone === key && <span style={{ color: Y, fontSize: 14 }}>✓</span>}
              </div>
            ))}
          </div>
        </div>
